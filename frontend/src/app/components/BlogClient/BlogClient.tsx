     ...
     >
        ...
        <ul className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-8"> 
            {Array.from({ length: 6 }).map((_, i) => (
              <li
                key={i}
                className=" border-2 border-[#57564F] p-4 shadow-2xl rounded-lg transition-transform duration-300 hover:scale-105 hover:shadow-[#7A7A73] bg-[#000]"
              >
                {posts[i] ? (
                  <Link href={`/blog/${posts[i].slug.current}`}>
                    <div
                      className="relative z-30 container mx-auto  w-full max-w-6xl"
                    >
                      {posts[i].image && (
                        <img
                          src={urlFor(posts[i].image)
                            .width(250)
                            .height(200)
                            .url()}
                          alt={posts[i].title}
